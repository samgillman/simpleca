...
ui <- dashboardPage(
  ...
  header = dashboardHeader(
    title = tags$div(
      style = "font-family: 'Inter', Arial, sans-serif; font-weight: 600; font-size: 18px;",
      HTML("SimpleCa<sup>2+</sup>")
    )
  ),
...
server <- function(input, output, session) {
  ...
  # Export module - this needs special handling for plot reactives
  # We'll create simple reactive wrappers for the plots
  metrics_plot_reactive <- reactive({
    # This will be populated by the metrics module if it returns a plot
    # For now, we'll use a simple approach
    req(rv$metrics)
    # Return a basic plot or NULL - the export module will handle this
    NULL
  })
  
  heatmap_plot_reactive <- reactive({
    # This will be populated by the heatmap module if it returns a plot  
    req(rv$dts)
    # Return a basic plot or NULL - the export module will handle this
    NULL
  })
...
# =============================== renv Setup ===============================
# renv should auto-activate from .Rprofile, but let's be explicit
if (!requireNamespace("renv", quietly = TRUE)) {
  # If renv is not available, try to install it
  install.packages("renv")
}

# Check if we're in an renv project and activate if needed
if (file.exists("renv.lock") && !identical(getOption("renv.project.initialized"), TRUE)) {
  renv::activate()
}
...
required_packages <- c(
  ...
  "shinycssloaders", "DT", "ggplot2", "dplyr", "tidyr",
  "data.table", "readxl", "purrr", "RColorBrewer", "scales",
  "colourpicker", "zoo", "shinyvalidate", "knitr", "kableExtra",
  "plotly", "bslib", "gt", "webshot2", "stringr", "tools"
)

# Install missing packages in renv environment
missing_packages <- required_packages[!sapply(required_packages, requireNamespace, quietly = TRUE)]
if (length(missing_packages) > 0) {
  cat("Installing missing packages in renv environment:", paste(missing_packages, collapse = ", "), "\n")
