...
sidebar <- dashboardSidebar(
  sidebarMenu(id = "sidebar_tabs",
              ...
              menuItem("Data Processing", tabName = "preproc", icon = icon("sliders")),
...
body <- dashboardBody(
  ...
  tabItems(
    tabItem(tabName = "load",
            fluidRow(
              box(title = "Load Data", status = "primary", solidHeader = TRUE, width = 8,
                  ...
                  br(), div(class="small-help","Upload raw/pre-processed traces; compute ΔF/F₀ in Data Processing if needed.")
              ),
              ...
              box(title = "Processing Status", status = "info", solidHeader = TRUE, width = 12,
                  div(id = "status_panel",
                      tags$div(class = "row",
                               tags$div(class = "col-sm-3", icon("file-import", class = "fa-2x"), h4("Files Loaded"), textOutput("status_files_loaded")),
                               tags$div(class = "col-sm-3", icon("check-circle", class = "fa-2x"), h4("Processing"), textOutput("status_processing")),
                               tags$div(class = "col-sm-3", icon("calculator", class = "fa-2x"), h4("Metrics"), textOutput("status_metrics")),
                               tags$div(class = "col-sm-3", icon("chart-line", class = "fa-2x"), h4("Ready"), textOutput("status_ready"))
                      )
                  ),
    ...
    tabItem(tabName = "tables",
            fluidRow(
              box(title = "Per-cell Metrics", status = "primary", solidHeader = TRUE, width = 12, DTOutput("metrics_table"))
            ),
            fluidRow(
              box(title = "Time-course Summary (Mean ± SEM)", status = "primary", solidHeader = TRUE, width = 12, DTOutput("summary_table"))
            )
    ),
...
server <- function(input, output, session) {
  ...
  # Value boxes & status
  output$vb_groups <- renderValueBox({ valueBox(length(rv$dts), "Files loaded", icon=icon("layer-group"), color="teal") })
  output$vb_cells <- renderValueBox({
    n <- sum(purrr::map_int(rv$dts, ~max(0, ncol(.x)-1)))
    valueBox(n, "Total cells", icon=icon("circle"), color="purple")
  })
  output$vb_timepoints <- renderValueBox({
    tp <- sum(purrr::map_int(rv$dts, nrow))
    valueBox(tp, "Total timepoints", icon=icon("clock"), color="olive")
  })
    ...
    build_hm <- function(dt, label) {
      ...
      if (isTRUE(input$hm_norm_per_cell)) {
        mat <- apply(mat, 2, function(x) {
          xr <- range(x, na.rm = TRUE); if (xr[2] > xr[1]) (x - xr[1])/(xr[2]-xr[1]) else rep(0.5, length(x))
        })
      }
